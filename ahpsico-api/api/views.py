...
from . import (authentication, enums, exceptions, models, permissions,
               serializers)
    ...
    * Requires token authentication.
    ...
    def get_permissions(self):
        if self.action == "retrieve":
            ...
            return [permissions.HasToken(), permissions.IsOwner()]
        return super().get_permissions()

    @action(detail=True, permission_classes=[permissions.HasToken, permissions.HasPatientInformation])
    def sessions(self, request, *args, **kwargs):
        uid = request.user.uid
        pk = kwargs["pk"]
